...
# /tmp is writable during a serverless function lifetime.
IS_VERCEL = os.getenv("VERCEL") == "1"

if IS_VERCEL:
    RUNTIME_DATA_DIR = Path("/tmp/finsentinels_runtime")
else:
    RUNTIME_DATA_DIR = DATA_DIR

CASE_FILE = RUNTIME_DATA_DIR / CASE_FILENAME
SETTINGS_FILE = RUNTIME_DATA_DIR / SETTINGS_FILENAME
...
# Keeps runtime changes available for the current process.
...
app = FastAPI(
    title="FinSentinels",
    description=(
        "Explainable graph-based financial fraud network "
        "detection and investigation API"
    ),
    version="3.0.0",
)
...
def read_json_file(
    ...
    """
    Read runtime state first, then fall back to bundled seed data.

    On Vercel:
        /tmp/finsentinels_runtime/<filename>
        ↓
        bundled data/<filename>
    ...
    # Runtime state must be checked FIRST.
    candidate_paths = [
        runtime_path,
        bundled_path,
    ]
...
def write_json_file(
    ...
    Memory is always updated even when filesystem persistence
    is unavailable.
    """
...
def get_settings() -> dict[str, Any]:
    ...
    settings["high_threshold"] = int(
        settings.get(
            "high_threshold",
            DEFAULT_SETTINGS["high_threshold"],
        )
    )
    ...
    settings["watch_threshold"] = int(
        settings.get(
            "watch_threshold",
            DEFAULT_SETTINGS["watch_threshold"],
        )
    )
...
def network(scope: str = "alerts"):
    try:
        ...
        if normalized == "full":
            ...
            # Fallback to WATCH when no HIGH RISK
            # accounts are available in the dataset.
            if not alert_ids:
                for aid, node_data in G.nodes(
                    data=True
                ):
...
def accounts():
    try:
        ...
        for account_id, node_data in G.nodes(
            data=True
        ):
            if node_data.get(
                "type"
...
def startup():
    """
    Do not create/write bundled data files on startup.

    On Vercel, the project filesystem is read-only.
    Runtime data is initialized lazily by read/write helpers.
    """
